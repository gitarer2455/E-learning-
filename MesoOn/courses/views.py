...
def krijo_klase(request):
    if not request.user.profile.is_teacher == True:
        messages.error(request, f'LYour account does not have access to this url, only teachers accounts!')
        return redirect('courses:home')
    if request.method == 'POST':
        form = KlasaForm(data=request.POST, files=request.FILES)
        if form.is_valid():
            form.save()
            messages.success(request, f'You were created blog successfully.')
            return redirect('courses:home')
    else:
        form = KlasaForm()
    context = {
...
def krijo_lende(request):
    if not request.user.profile.is_teacher == True:
        messages.error(request, f'Your account does not have access to this url, only teachers accounts!')
        return redirect('courses:home')
    if request.method == 'POST':
